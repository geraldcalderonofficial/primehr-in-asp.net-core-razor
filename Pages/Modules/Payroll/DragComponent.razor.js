...
function renderGrid(container, index) {
	let grid = $('<div class="demo-grid" id="grid' + index + '"></div>').appendTo(container);
	grid.dxDataGrid({
		...
		rowDragging: {
			...
			data: index,
			onAdd: function (e) {
				let selectedRowKeys = e.fromComponent.getSelectedRowKeys();
				for (key of selectedRowKeys) {
					const values = { Priority: e.toData };
			...
			onDragStart: function (e) {
				let selectedRowKeys = e.component.getSelectedRowKeys(),
					selectedRowElements = getSelectedRowElements(selectedRowKeys, e.component),
			...
			onDragEnd: function (e) {
				e.component._selectedRowElements.forEach((rowEl) => {
					rowEl.classList.remove("dx-sortable-source")
				});
			},
			dragTemplate: function (e) {
				let sourceIndex = e.itemData.Priority,
					sourceGrid = $("#grid" + sourceIndex).dxDataGrid("instance"),
					selectedRowKeys = sourceGrid.getSelectedRowKeys(),
					selectedTasks = tasks.filter((task) => {
						for (key of selectedRowKeys)
							if (task.ID === key) return true;
						return false;
					});
				const $container = $("<div/>");
				renderDragged(selectedTasks, $container);
				return $container;
			},
		},
	});
...
$(function () {
	renderGrid($("#gridContainer"), 1);
	renderGrid($("#gridContainer"), 2);
});

function getSelectedRowElements(keys, component) {
	let arr = [],
		idx = null,
		el;
	keys.forEach((key) => {
		idx = component.getRowIndexByKey(key);
		el = component.getRowElement(idx)[0];
		arr.push(el);
	});
	return arr;
};

function renderDragged(selectedTasks, $container) {
...
tasksStore = new DevExpress.data.ArrayStore({
	data: tasks,
	key: "ID",
});